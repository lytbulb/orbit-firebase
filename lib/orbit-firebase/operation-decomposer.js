...
export default Class.extend({
	...
	decompose: function(operation){
		...
		if(operation.path.length === 2 && operation.op === 'add'){
			operations.push.apply(operations, this.resolveRecordLinkOperations(operation));
		}
		else if (operation.path[2] === '__rel'){
			operations.push.apply(operations, this.resolveInverseOperations(operation))
		}
		
		var filtered = filterRedundentOperations(operations);
		var withParents = this.addParentOperations(operations, operation);
	...
	resolveRecordLinkOperations: function(operation){
		...
		Object.keys(record.__rel||{}).forEach(function(link){
			var linkOperation = {op: 'replace', path: [type, record.id, '__rel', link], value: record.__rel[link]};
			var operations = _this.resolveInverseOperations(linkOperation);
			resolved.push.apply(resolved, operations);
		});
	...
	resolveInverseOperations: function(operation){
		...
		var operations = relationshipResolver.operations;
		operations.shift();
		return operations;
	},
