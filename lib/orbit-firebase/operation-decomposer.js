...
var RelationshipResolver = Class.extend({
	...
	hasOne: {
		add: function(path, value){
			...
			this.operations.push({ op: 'add', path: changeDetails.path, value: changeDetails.value });
		...
		remove: function(path, value){
			var changeDetails = new ChangeDetails(path, value, this.schema, this.cache);
			if(!value) return;
			this.operations.push({ op: 'remove', path: changeDetails.path});
			this.visit("remove", changeDetails.originalInversePath(), changeDetails.modelId());
		},
		...
		replace: function(path, value){
			...
			this.operations.push({ op: 'replace', path: changeDetails.path, value: changeDetails.value });
			if(changeDetails.currentValue()){
				this.visit("remove", changeDetails.originalInversePath(), changeDetails.modelId());
			}
			this.visit("add", changeDetails.newInversePath(), changeDetails.modelId());
	...
	hasMany: {
		add: function(path, value){
			...
			this.operations.push({ op: 'add', path: path.concat(value), value: true });
		...
		remove: function(path, value){
			var linkDef = this.schemaUtils.lookupLinkDef(path[0], path[3]);
			var inversePath = [linkDef.model, value, "__rel", linkDef.inverse];
			this.operations.push({ op: 'remove', path: path.concat(value) });
		...
		replace: function(path, value){
			...
			this.operations.push({ op: 'replace', path: path, value: buildObject(value, true) });
...
export default Class.extend({
	...
	decompose: function(operation){
		...
		var withRecordLinkOperations = this.addRecordLinkOperations(operations);
		console.log("withRecordLinkOperations", withRecordLinkOperations);
		var withInverses = this.addInverseOperations(withRecordLinkOperations);
		console.log("withInverses", withInverses);

		var withParentOperations = this.addParentOperations(withInverses, operation);

		return withParentOperations;
	},

	addParentOperations: function(operations, parentOperation){
		var resolved = [];
		...
		operations.forEach(function(operation){
			if(operation.parent){
				...
				resolved.push(parentOperation.spawn(operation));
			}
		});
	...
	addRecordLinkOperations: function(operations){
		var _this = this;
		var resolved = [];

		operations.forEach(function(operation){
			console.log("addRecordLinkOperation", operation);
			if(operation.path.length === 2 && operation.op === 'add'){
				resolved.push.apply(resolved, _this.resolveRecordLinkOperations(operation));
			}
			else {
				resolved.push(operation);
			}
	...
	resolveRecordLinkOperations: function(operation){
		var record = operation.value;
		var type = operation.path[0];

		var resolved = [operation];
		Object.keys(record.__rel||{}).forEach(function(link){
			var op = {op: 'replace', path: [type, record.id, '__rel', link], value: record.__rel[link]};
			resolved.push(operation.spawn(op));
			console.log("reoslved", resolved);
		});

		return resolved;
	},

	addInverseOperations: function(operations){
		var _this = this;
		var resolved = [];

		operations.forEach(function(operation){
			if(operation.path[2] === '__rel') {
				resolved.push.apply(resolved, _this.resolveInverseOperations(operation));
			}
			else {
				resolved.push(operation);
			}
	...
	resolveInverseOperations: function(operation){
