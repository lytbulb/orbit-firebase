...
export default Class.extend({
	...
	subscribeToRecord: function(type, id, options){
		...
		this._addSubscription2(path, RecordSubscription, options);
	},
	...
	_subscribeToAttribute: function(type, id, attribute){
		...
		this._addSubscription2(path, AttributeSubscription);
	},
	...
	_subscribeToHasOne: function(type, id, link){
		...
		return this._addSubscription2(path, HasOneSubscription);
	},
	...
	_subscribeToHasMany: function(type, id, link){
		...
		return this._addSubscription2(path, HasManySubscription);
	},

	_addSubscription2: function(path, subscriptionClass, options){
		console.log("_addSubscription2", path);
		var subscription = this._findSubscription(path);

		if(subscription){
			console.log("subscription:existing", path);
			return subscription.update(options);
		}
		else {
			console.log("subscription:activating", path);
			subscription = new subscriptionClass(path, this);
			this._addSubscription(path, subscription);
			return subscription.activate(options);
		}
	},
