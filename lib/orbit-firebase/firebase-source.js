...
export default Source.extend({
	...
	_find: function(type, id, options){
		...
		return this._firebaseRequester.find(type, id).then(function(records){
			if(!id) _this._firebaseListener.subscribeToType(type, options);
			return _this._subscribeToRecords(type, records, options).then(function(){
				return records;
			});
		});
	},
	...
	_findLinked: function(type, id, link){
		...
		return this._firebaseRequester.findLinked(type, id, link).then(function(records){
			return _this._subscribeToRecords(linkedType, records).then(function(){
				return records;
			});
		});
	},
