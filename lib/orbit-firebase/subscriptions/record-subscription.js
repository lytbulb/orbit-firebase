...
export default Class.extend({
	...
	activate: function(){
		...
		console.group("Subscribing to record", [type, recordId].join("/"));
		...
		var linkSubscriptionPromises = options.currentIncludes().map(function(link){
			console.log("subscription:adding link to " + [type, recordId].join("/"), link);
			return listener._subscribeToLink(type, recordId, link, options.forLink(link)).then(function(result){
				console.log("subscription:added link to " + [type, recordId].join("/"), link);
				return result;
			});
		});
		...
		return Orbit.all([
			...
			Orbit.all(linkSubscriptionPromises).then(function(result){
				console.log("subscription:added links to", [type, recordId].join("/"));
				return result;
			})
		]).then(function(){
			console.groupEnd();
