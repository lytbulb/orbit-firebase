...
import { LinkNotInitializedException } from 'orbit-common/lib/exceptions';
...
export default TransformConnector.extend({
  ...
  _modifyHasManyContents: function(operation, record){
    ...
    if(linkValue === OC.LINK_NOT_INITIALIZED) {
      throw new LinkNotInitializedException(operation.path[0], operation.path[1], operation.path[3]);
    }
